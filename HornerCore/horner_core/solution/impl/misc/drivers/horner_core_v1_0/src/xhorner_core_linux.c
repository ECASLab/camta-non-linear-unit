...
typedef struct {
    ...
    XHorner_core_uio_map maps[ MAX_UIO_MAPS ];
} XHorner_core_uio_info;

/***************** Variable Definitions **************************************/
static XHorner_core_uio_info uio_info;
...
static int uio_info_read_name(XHorner_core_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XHorner_core_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XHorner_core_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XHorner_core_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XHorner_core_Initialize(XHorner_core *InstancePtr, const char* InstanceName) {
	XHorner_core_uio_info *InfoPtr = &uio_info;
